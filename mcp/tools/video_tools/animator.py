...
def get_ffmpeg_zoompan_filter(effect: str, duration_seconds: float) -> str:
    """
    Build FFmpeg zoompan filter string for a tone effect with professional Ken Burns style.
    """
    frames = max(1, int(round(duration_seconds * FPS)))
    safe_frames = frames + 24 # Pad frames so zoompan never stops early
    res = "512x512"
    
    if effect == "zoom_in":
        # smooth slow zoom from 1.0x to 1.3x over full duration
        return f"zoompan=z='min(zoom+0.0015,1.3)':d={safe_frames}:s={res}:fps={FPS}"
    if effect == "zoom_out":
        # starts at 1.3x zoom, slowly pulls back to 1.0x
        return f"zoompan=z='if(eq(on,1),1.3,zoom-0.0015)':d={safe_frames}:s={res}:fps={FPS}"
    if effect == "pan_left_zoom":
        # zoom 1.2x + pan from right to left
        return f"zoompan=z=1.2:x='(iw-iw/zoom)*(1-on/{frames})':d={safe_frames}:s={res}:fps={FPS}"
    if effect == "pan_right_zoom":
        # zoom 1.2x + pan from left to right
        return f"zoompan=z=1.2:x='(iw-iw/zoom)*(on/{frames})':d={safe_frames}:s={res}:fps={FPS}"
    if effect == "pan_up":
        # zoom 1.2x + slow upward pan
        return f"zoompan=z=1.2:y='(ih-ih/zoom)*(1-on/{frames})':d={safe_frames}:s={res}:fps={FPS}"
    if effect == "pan_down":
        # zoom 1.2x + slow downward pan
        return f"zoompan=z=1.2:y='(ih-ih/zoom)*(on/{frames})':d={safe_frames}:s={res}:fps={FPS}"
    if effect == "dramatic_push":
        # fast zoom from 1.0x to 1.5x (for tense moments)
        return f"zoompan=z='min(zoom+0.01,1.5)':d={safe_frames}:s={res}:fps={FPS}"
        
    return f"zoompan=z='zoom+0.0015':d={safe_frames}:s={res}:fps={FPS}"
...
def animate_scene(
    ...
    # Combine filters: zoompan, color (if any), vignette. NO FADE to prevent black gaps between clips.
